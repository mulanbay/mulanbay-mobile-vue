...
<template>
  <div>
    <top-bar>
      <span slot="topText" class="top-text">身体不适</span>
    </top-bar>
    <van-search
      v-model="para.name"
      show-action
    ...
    <van-pull-refresh v-model="pullLoading" @refresh="resetSearch">
      ...
      <van-cell-group v-for="(item, index) in dataList" :key="index" >
        <van-swipe-cell>
          <van-cell center is-link :to="{ name:'BodyAbnormalRecordEdit',params: {id:item.id} }">
            <!-- 使用 title 插槽来自定义标题 -->
            <template #icon>
              <svg-icon icon-class="sick" className="icon-list" />
            </template>
            <template #title>
              <span class="custom-title">{{item.disease}}</span>
            </template>
            <template #default>
              <span class="van-ellipsis">{{item.organ}}</span>
            </template>
          </van-cell>
          <template #right>
        ...
        <van-cell center class="custom-cell" title="开始日期" :value="item.occurDate" />
        <van-cell center class="custom-cell" title="结束日期" :value="item.finishDate" />
        <van-cell center class="custom-cell" title="持续天数" :value="item.lastDays" />
        <van-cell :value="item.remark" value-class="desc-class"/>
      </van-cell-group>
      </van-list>
    </van-pull-refresh>
    <van-button round block type="primary" class="btn-block" :to="{ name:'BodyAbnormalRecordAdd',params: {} }" >
        新增
    </van-button>
    <!--回到顶部-->
...
<script>
  import { fetchList,deleteBodyAbnormalRecord } from '@/api/health/body/bodyAbnormalRecord'
  ...
  methods: {
    ...
    handleDelete(id) {
      Dialog.confirm({
        ...
        //确认
        deleteBodyAbnormalRecord(id).then(response => {
